   ...
   stage('Image Build'){
        ...
        stage('Remove Build Images'){
         ...
         stage('Deploy in server'){
                       def dockerpull = 'sudo docker pull suzuran1995/abc:1.${BUILD_NUMBER}'
                      def dockerContKill = 'sudo docker kill abc || true'
                      def dockerContRm = 'sudo docker rm -f abc || true'
                      def dockerContRun =  'sudo docker run -d --name abc -p 8585:8585 -e SPRING_PROFILES_ACTIVE=dev suzuran1995/abc:1.${BUILD_NUMBER}'
                    sshagent(['server']) {
